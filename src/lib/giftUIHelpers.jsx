 ...
 * @param {Object} params.giftData - Gift data { gift_id, gift_name, message, coins_spent, gems_earned }
 * @param {Function} params.setWallet - State setter for wallet
 * @param {Function} params.showToast - Toast notification function
 * @param {Function} params.insertGiftMessage - Function to insert message into chat (optional)
...
export async function handleGiftSendSuccess({
  ...
  // 2. Update wallet state with returned data
  if (result.sender_wallet && setWallet) {
    console.log('💰 Updating wallet:', result.sender_wallet);
    setWallet({
  ...
  // 3. Insert gift message into chat (if function provided)
  if (insertGiftMessage && giftData) {
    try {
      console.log('💬 Inserting gift message into chat...');
      await insertGiftMessage({
        gift_id: giftData.gift_id,
        gift_name: giftData.gift_name,
        message: giftData.message,
        coins_spent: giftData.coins_spent,
        gems_earned: giftData.gems_earned,
      });
    } catch (err) {
      console.error('⚠️ Error inserting gift message:', err);
 ...
 * 
 * @param {Object} params
 * @param {Object} params.result - Error response from sendGiftSecure
 * @param {Function} params.showToast - Toast notification function
...
export function handleGiftSendError({
  ...
  // Convert action object to toast expected format if needed, or pass customized action component
  // Since we are using shadcn toast, we usually pass an 'action' prop which is a React node.
