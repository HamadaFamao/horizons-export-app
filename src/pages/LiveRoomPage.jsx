...
const isSmallRoomGift = (effect) => {
  // ✅ نعتمد على display_size من الـ DB مباشرة
  if (effect?.display_size === 'large' || effect?.display_size === 'fullscreen') {
    return false;
  }
  if (effect?.display_size === 'small') {
    return true;
  }
  // fallback بالسعر لو مفيش display_size
...
.gift-bounce {
  /* Removed: emoji animation CSS (wiggle, bounce, pulse, spin, shake, float) -- now handled by Lottie */
@keyframes marquee {
      ...
      .on(
        ...
        ({ payload }) => {
          ...
          setLargeGiftBanner({
            ...
            teamWinnerNames: payload.team_winner_names || null,
            teamWinnerAvatars: payload.team_winner_avatars || null,
...
useEffect(() => {
  ...
  // Removed all legacy emoji animation references; Lottie handles all animation now.
 ...
 useEffect(() => {
  ...
  if (payload.action === 'clear_background') {
    setRoom(prev => prev ? { ...prev, background_url: null } : prev);
      ...
      ch.on("broadcast", { event: "room_follow_changed" }, ({ payload }) => {
        if (payload?.room_id && String(payload.room_id) !== String(roomId)) return;
          ...
          if (payload?.profile) {
            setFollowersList((prev) => {
              const exists = prev.some(
                (f) => String(f.user_id) === String(payload.user_id)
              );
              ...
              return [{
                ...
                is_vip: !!payload.profile.is_vip,
                vip_number: payload.profile.vip_number || null,
        ...
        if (payload?.action === 'unfollow') {
          ...
          setFollowersList((prev) =>
            prev.filter((f) =>
              String(f.user_id) !== String(payload.user_id)
            )
          );
        ...
        onLudoResult={({ winnerName, winnerAvatar,
          ...
          if (bigWinValue >= 5000) {
            ...
            setLargeGiftBanner({
              ...
              giftIcon: null,
              teamWinnerNames: teamMode ? teamNames : null,
              teamWinnerAvatars: teamMode ? teamAvatars : null,
              isToAll: false,
              roomId: roomId,
            ...
            if (globalGiftChannelRef.current) {
              globalGiftChannelRef.current.send({
                ...
                payload: {
                  ...
                  team_winner_names: teamMode ? teamNames : null,
                  team_winner_avatars: teamMode ? teamAvatars : null,
